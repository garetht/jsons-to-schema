...
unifySchemas nextSchema =
  ...
  unifyNumericConstraints nextSchema . unifyAnyInstanceConstraints nextSchema . unifyNonvalidatingConstraints nextSchema
...
unifyNonvalidatingConstraints nextSchema accSchema =
  accSchema
...
unifyStringConstraints nextSchema accSchema =
  accSchema
...
unifyArrayConstraints nextSchema accSchema =
  accSchema
  ...
  where
    ...
    uAdditional (V4Arr.AdditionalBool b1) (V4Arr.AdditionalBool b2) = V4Arr.AdditionalBool $ b1 || b2
    ...
    uAdditional bln@(V4Arr.AdditionalBool b) obj@(V4Arr.AdditionalObject s) =
      if b
        then bln
        else obj
    uAdditional obj@(V4Arr.AdditionalObject s) bln@(V4Arr.AdditionalBool b) =
      if b
        then bln
        else obj
    uAdditional (V4Arr.AdditionalObject o1) (V4Arr.AdditionalObject o2) = V4Arr.AdditionalObject $ unifySchemas o1 o2
...
unifyObjectConstraints nextSchema accSchema =
  accSchema
  { D4._schemaRequired =
      Utils.setToMaybeSet =<< -- to avoid empty lists, which are not allowed
      applicativeUnifier DS.intersection D4._schemaRequired nextSchema accSchema
  , D4._schemaProperties = altUnifier (HM.unionWith unifySchemas) D4._schemaProperties nextSchema accSchema
  , D4._schemaAdditionalProperties = altUnifier unify D4._schemaAdditionalProperties nextSchema accSchema
  ...
  , D4._schemaPatternProperties =
      altUnifier (HM.unionWith unifySchemas) D4._schemaPatternProperties nextSchema accSchema
  }
  where
    unify ::
...
unifyAnyInstanceConstraints nextSchema accSchema =
  accSchema
